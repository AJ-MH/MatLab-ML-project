...
clear all
close all
fileName = 'All_Named_UnNormalized_BubbleEntropy_Features.xlsx'; 
...
for i = 1:15
    eval(sprintf('mean_bubble(%d,:) = mean(data%d);', i, i));
    eval(sprintf('sd_bubble(%d,:) = std(data%d);', i, i));
end
...
% Save P-Values
p_values_filename = 'p_values_BubbleEn_WPT_15Features.xlsx';
...
feature_names = [ ...
    strcat(feature_labels, "_mean_bubble"), ...
    strcat(feature_labels, "_sd_bubble")];

mean_std_data = [mean_bubble; sd_bubble];
...
mean_std_filename = 'mean_std_BubbleEn_WPT_15Features.xlsx';
