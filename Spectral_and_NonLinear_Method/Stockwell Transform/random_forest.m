...
% Load the dataset
time_data = readmatrix('All_UnNormalized_SampleAndBubbleEntropy_Features.xlsx'); % Assuming your data is stored as table
...
headers = {'Accuracy', 'Recall', 'Specificity', 'Precision', 'F_Score'};

% Convert class labels and metrics to a cell array
class_labels = num2cell([metrics.Class]');
metric_values = num2cell(para);

% Create the final cell array with headers
final_metrics = [['Class', headers]; [class_labels, metric_values]];

% Write to Excel
xlswrite('RandomForest_Metrics_BubbleAndSample.xlsx', final_metrics);

disp('Metrics saved to RandomForest_Metrics_BubbleAndSample.xlsx');
